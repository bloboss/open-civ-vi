...
use libciv::civ::great_people::{
    GP_PATRONAGE_GOLD_PER_POINT, next_candidate_name, recruitment_threshold,
};
use libciv::civ::{
    BasicUnit, BuiltinAgenda, City, Civilization, Leader, ProductionItem, TechProgress, Unit,
};
...
use libciv::{
    BarbarianCampId, BeliefId, BuiltinVictoryCondition, CityId, CivId, DefaultRulesEngine,
    GameState, GameStateDiff, GreatPersonType, RulesEngine, TurnEngine, UnitCategory, UnitDomain,
    UnitId, UnitTypeId, all_scores,
};
...
fn main() {
    ...
    match parsed.command {
        ...
        cli::Command::NewGame {
            ...
            if let Err(e) =
                handlers::new_game::handle_new_game(&game_file, seed, width, height, &player, &ai)
            {
                eprintln!("Error: {e}");
                std::process::exit(1);
            }
...
fn build_session() -> Session {
    ...
    state
        .civilizations
        .push(Civilization::new(civ_id, "Rome", "Roman", leader));
...
fn civ_summary_line(demo: &AiDemo, civ_id: CivId) -> String {
    ...
    let prod_str = cities
        ...
        .and_then(|item| match item {
            ProductionItem::Unit(tid) => {
                state.unit_type_defs.iter().find(|d| d.id == *tid).map(|d| {
                    format!(
                        "{}[{}/{}]",
                        capitalize(d.name),
    ...
    format!(
        "  {:8}  cities={:2}  pop={:2}  units={:2}  explored={:3}  prod={}",
        civ.name,
        cities.len(),
        total_pop,
        units,
...
fn run_ai_demo(turns: u32, seed: u64, board_every: u32) {
    ...
    let rome_exp = demo
        ...
        .civilizations
        .iter()
        .find(|c| c.id == demo.rome_id)
        .map(|c| c.explored_tiles.len())
        .unwrap_or(0);
    let babylon_exp = demo
        .state
        .civilizations
        .iter()
        .find(|c| c.id == demo.babylon_id)
        .map(|c| c.explored_tiles.len())
        .unwrap_or(0);
    println!(
        "  Map coverage:  Rome {}/{} ({:.0}%)   Babylon {}/{} ({:.0}%)",
        rome_exp,
        total_tiles,
        100.0 * rome_exp as f64 / total_tiles as f64,
        babylon_exp,
        total_tiles,
        100.0 * babylon_exp as f64 / total_tiles as f64,
    );
    println!();
    print_ai_board(&demo);
}
            ...
            match parse_cmd(&line) {
                        ...
                        Some(uid) => {
                            ...
                            match rules.move_unit(&session.state, uid, target) {
                                ...
                                Err(RulesError::InsufficientMovement(diff)) if !diff.is_empty() => {
                                    ...
                                    let after = session.state.unit(uid).unwrap().coord();
                                    println!(
                                        "  [partial] Unit moved {} -> {}  (ran out of movement)",
                                        fmtc(before),
                                        fmtc(after)
                                    );
                                    println!(
                                ...
                                Err(RulesError::DestinationImpassable) => {
                                    println!(
                                        "  [error] {} is impassable or unreachable.",
                                        fmtc(target)
                                    );
                                }
                ...
                Cmd::Status => {
                    let city = session
                        ...
                        .cities
                        .iter()
                        .find(|c| c.id == session.city_ids[session.current_city])
                        .unwrap();
                    println!(
                        "  Turn {}  |  Roma pop={}  food {}/{}",
                ...
                Cmd::Settle(name) => {
                    ...
                    match session.selected_unit {
                        ...
                        Some(uid) => match rules.found_city(&mut session.state, uid, name) {
                            Ok(diff) => {
                ...
                Cmd::Attack(q, r) => {
                    ...
                    match session.selected_unit {
                        ...
                        Some(uid) => {
                            ...
                            match defender {
                                None => {
                                    println!("  [error] No enemy unit at {}.", fmtc(target_coord))
                                }
                                Some(def_id) => {
                                                ...
                                                match delta {
                                                    StateDelta::UnitAttacked {
                                                        ...
                                                        let type_str = match attack_type {
                                                            ...
                                                            AttackType::CityBombard => {
                                                                "CityBombard"
                                                            }
                                                        };
                                                        println!(
                                                            "  [{type_str}] dealt {defender_damage} dmg, took {attacker_damage} dmg"
                                                        );
                                                    }
                ...
                Cmd::Techs => {
                    let civ = session
                        ...
                        .civilizations
                        .iter()
                        .find(|c| c.id == session.civ_id)
                        .unwrap();
                    println!(
                        "  Researched technologies ({}):",
                        civ.researched_techs.len()
                    );
                ...
                Cmd::Switch(n) => {
                    if n == 0 || n > session.city_ids.len() {
                        println!(
                            "  [error] City index out of range (1-{}).",
                            session.city_ids.len()
                        );
                    } else {
                ...
                Cmd::Trade(q, r) => {
                    ...
                    match session.selected_unit {
                        None => println!(
                            "  [error] No unit selected. Select a trader with 'select <q> <r>'."
                        ),
                        Some(uid) => {
                            let dest = session
                                .state
                                            ...
                                            for delta in &diff.deltas {
                                                ...
                                                } = delta
                                                {
                                                    ...
                                                    let dest_name = session
                                                        ...
                                                        .unwrap_or("?");
                                                    println!(
                                                        "  Trade route established: {orig} -> {dest_name}!"
                                                    );
                                                }
                                            }
                                            if session.state.unit(uid).is_none() {
                ...
                Cmd::BarbIncite(q, r, target) => {
                    cmd_barb_incite(&mut session, &rules, q, r, &target)
                }
                Cmd::FaithBuy(name) => cmd_faith_buy(&mut session, &rules, &name),
                Cmd::GovernorPromote(name, promo) => {
                    cmd_governor_promote(&mut session, &rules, &name, &promo)
                }
                ...
                Cmd::Perform => match session.selected_unit {
                    None => println!("  [error] No unit selected."),
                    Some(uid) => match rules.rock_band_perform(&mut session.state, uid) {
                        Ok(diff) => {
                            for delta in &diff.deltas {
                                match delta {
                                    StateDelta::RockBandPerformed { tourism_gained, .. } => {
                                        println!(
                                            "  Rock Band performed! Tourism gained: {tourism_gained}"
                                        );
                                    }
                                    StateDelta::UnitDestroyed { .. } => {
...
fn parse_cmd(raw: &str) -> Cmd {
    ...
    match parts.as_slice() {
        ["move" | "m", q, r] | ["go", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::Move(q, r))
        ...
        ["quit" | "exit"] => Cmd::Quit,
        ["help" | "h" | "?"] => Cmd::Help,
        ["tile" | "t", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::Tile(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["city" | "c"] => Cmd::City,
        ["cities" | "cs"] => Cmd::Cities,
        ["assign" | "a", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::Assign(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["lock", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::Lock(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["unassign" | "ua", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::Unassign(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ...
        ["yields" | "y"] => Cmd::Yields,
        ["units" | "u"] => Cmd::Units,
        ["select" | "sel", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::Select(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["settle" | "found", rest @ ..] => Cmd::Settle(rest.join(" ")),
        ["attack" | "atk", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::Attack(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["techs"] => Cmd::Techs,
        ["civics"] => Cmd::Civics,
        ["switch", n] => n
        ...
        ["trade" | "tr", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::Trade(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["routes" | "rt"] => Cmd::Routes,
        ["scores" | "sc"] => Cmd::Scores,
        ["congress" | "wc"] => Cmd::Congress,
        ["perform"] => Cmd::Perform,
        ["district" | "dist", name, q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::District(name.to_string(), q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["road", q, r] => parse_coord(q, r)
        ...
        ["religion" | "rel", name, b1, b2] => {
            Cmd::Religion(name.to_string(), b1.to_string(), b2.to_string())
        }
        ["great-people" | "gp"] => Cmd::GreatPeople,
        ["recruit", rest @ ..] if !rest.is_empty() => Cmd::Recruit(rest.join(" ")),
        ["governor", name, idx] => idx
            .parse::<usize>()
            .ok()
        ...
        ["spread"] => Cmd::Spread,
        ["theo-combat" | "tc", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::TheoCombat(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["bombard", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::Bombard(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["retire", rest @ ..] if !rest.is_empty() => Cmd::Retire(rest.join(" ")),
        ["great-work" | "gw", rest @ ..] if !rest.is_empty() => Cmd::GreatWork(rest.join(" ")),
        ["recruit-faith" | "rf", rest @ ..] if !rest.is_empty() => {
            Cmd::RecruitFaith(rest.join(" "))
        }
        ...
        ["claim", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::ClaimTile(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["reassign", q, r, city_idx] => {
            if let (Some((q, r)), Some(idx)) = (parse_coord(q, r), city_idx.parse::<usize>().ok()) {
                Cmd::ReassignTile(q, r, idx)
            } else {
                Cmd::Unknown(s.to_string())
            }
        }
        ["send-trader" | "st", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::SendTrader(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["barb-hire" | "bh", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::BarbHire(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
        ["barb-bribe" | "bb", q, r] => parse_coord(q, r)
            .map(|(q, r)| Cmd::BarbBribe(q, r))
            .unwrap_or(Cmd::Unknown(s.to_string())),
...
fn print_help() {
    ...
    println!(
        "    governor-promote <g> <p> promote governor                       alias:  gp-promote"
    );
...
fn cmd_improve(session: &mut Session, rules: &DefaultRulesEngine, name: &str) {
    ...
    let improvement = match name.to_ascii_lowercase().replace(' ', "_").as_str() {
        ...
        _ => {
            ...
            println!(
                "  Valid: farm, mine, lumber_mill, trading_post, fort, airstrip, missile_silo"
            );
            return;
        }
    };
...
fn cmd_research(session: &mut Session, name: &str) {
    ...
    if !session
        .state
        .tech_tree
        .prerequisites_met(tech_id, &researched)
    {
...
fn cmd_tile(state: &GameState, q: i32, r: i32) {
    ...
    let feature_name = tile
        .feature
        .map(|f| f.name().to_string())
        .unwrap_or_else(|| "none".to_string());
    let resource_name = tile
        .resource
        .map(|r| format!("{r:?}"))
        .unwrap_or_else(|| "none".to_string());
    let improvement_name = tile
        .improvement
    ...
    let worked_by = state
        .cities
        .iter()
        .find(|c| c.worked_tiles.contains(&coord));
    ...
    // Units on this tile.
    let units_here: Vec<String> = state
        .units
        .iter()
    ...
    println!(
        "  Yields     : food={}  prod={}  gold={}  sci={}  cul={}  faith={}",
...
fn cmd_city(session: &Session, rules: &DefaultRulesEngine) {
    ...
    let prod_bar = match prod_cost {
        Some(cost) => format!(
            "{}  {} / {}",
            progress_bar(city.production_stored, cost),
            city.production_stored,
            cost
        ),
    ...
    let food_bar = format!(
        "{}  {} / {}",
        progress_bar(city.food_stored, city.food_to_grow),
        city.food_stored,
        city.food_to_grow
    );
    println!("  Food      : {food_bar}");
    let prod_label = prod_name
        .map(|n| format!("  [{}]", capitalize(n)))
        .unwrap_or_default();
    ...
    for coord in &city.worked_tiles {
        ...
        let center_tag = if *coord == city.coord {
            " [center]"
        } else {
            ""
        };
        let lock_tag = if city.locked_tiles.contains(coord) {
            " [locked]"
        } else {
            ""
        };
    ...
    let mut available: Vec<HexCoord> = (1u32..=3)
        ...
        .filter(|coord| state.board.tile(*coord).is_some() && !city.worked_tiles.contains(coord))
        .collect();
...
fn cmd_build(session: &mut Session, name: &str) {
    ...
    if let Some(def) = avail_buildings
        .iter()
        .find(|d| d.name.to_lowercase() == key)
    {
        ...
        let city = session
            ...
            .unwrap();
        city.production_queue
            .push_back(ProductionItem::Building(bld_id));
        println!(
            "  Queued: {} ({} prod) in {}",
            capitalize(bld_name),
            bld_cost,
        ...
        .find(|d| d.name.to_lowercase() == key)
    {
        ...
        let city = session
            ...
            .unwrap();
        city.production_queue
            .push_back(ProductionItem::Project(proj_id));
        println!(
            "  Queued: {} ({} prod) in {}",
            capitalize(proj_name),
            proj_cost,
            city.name
        );
        return;
    ...
    let all_unit_match = session
        .state
        .unit_type_defs
        .iter()
        .any(|d| d.name.to_lowercase() == key);
    let all_bldg_match = session
        .state
        .building_defs
        .iter()
        .any(|d| d.name.to_lowercase() == key);
    if all_unit_match || all_bldg_match {
        println!(
            "  [error] {key:?} is not yet available (requires tech unlock or is exclusive to another civ)."
        );
    } else {
        ...
        println!(
            "  [error] Unknown unit/building {:?}. Available: {}",
            key,
            available.join(", ")
        );
    }
...
fn cmd_cancel(session: &mut Session) {
    ...
    match city.production_queue.pop_front() {
        ...
        Some(item) => {
            ...
            println!(
                "  Cancelled: {}. Production reset to 0.",
                item_name_str(item)
            );
        }
    }
...
fn cmd_units(session: &Session) {
    ...
    for (i, unit) in owned.iter().enumerate() {
        let sel = if session.selected_unit == Some(unit.id) {
            "  [selected]"
        } else {
            ""
        };
...
fn cmd_select(session: &mut Session, q: i32, r: i32) {
    ...
    match session
        ...
        .units
        .iter()
        .find(|u| u.owner == civ_id && u.coord() == coord)
    {
...
fn cmd_district(session: &mut Session, rules: &DefaultRulesEngine, name: &str, q: i32, r: i32) {
    ...
    let district = match name.to_ascii_lowercase().replace(' ', "_").as_str() {
        ...
        "entertainment_complex" | "entertainmentcomplex" | "entertainment" => {
        ...
        _ => {
            ...
            println!(
                "         encampment, industrial_zone, entertainment_complex, aqueduct, water_park"
            );
            return;
        }
    };
...
fn cmd_government(session: &mut Session, name: &str) {
    ...
    let Some((_gov_id, gov_name)) = found else {
        ...
        if available.is_empty() {
            ...
            println!(
                "  [error] Unknown government '{name}'. Available: {}",
                available.join(", ")
            );
        }
        return;
...
fn cmd_pantheon(session: &mut Session, rules: &DefaultRulesEngine, name: &str) {
    ...
    let Some((belief_id, belief_name)) = found else {
        ...
        if available.is_empty() {
            ...
            println!(
                "  [error] Unknown belief '{name}'. Available: {}",
                available.join(", ")
            );
        }
        return;
...
fn cmd_great_people(session: &Session) {
    ...
    for gpt in &types {
        ...
        if let Some(candidate) = next_candidate_name(*gpt, state) {
            ...
            println!(
                "    {:10}  {:<20}  points: {}/{}  recruit cost: {} gold",
                format!("{:?}", gpt),
                candidate,
                current,
                threshold,
...
fn cmd_recruit(session: &mut Session, rules: &DefaultRulesEngine, type_name: &str) {
    let person_type = match type_name.to_ascii_lowercase().as_str() {
        ...
        _ => {
            ...
            println!(
                "  Valid: general, admiral, engineer, merchant, scientist, writer, artist, musician, prophet"
            );
            return;
        }
    };
    match rules.recruit_great_person(&mut session.state, session.civ_id, person_type) {
        Ok(diff) => {
            for delta in &diff.deltas {
                if let StateDelta::GreatPersonPatronized {
                    great_person,
                    gold_spent,
                    ..
                } = delta
...
fn cmd_governor(session: &mut Session, rules: &DefaultRulesEngine, name: &str, city_idx: usize) {
    ...
    let Some(governor_id) = governor else {
        ...
        if available.is_empty() {
            ...
            println!(
                "  [error] Unknown governor '{name}'. Available: {}",
                available.join(", ")
            );
        }
        return;
...
fn cmd_diplomacy(session: &Session) {
    ...
    for other in &other_civs {
        let relation = state.diplomatic_relations.iter().find(|r| {
            (r.civ_a == civ_id && r.civ_b == other.id) || (r.civ_b == civ_id && r.civ_a == other.id)
        });
...
fn print_turn_header(session: &Session, rules: &DefaultRulesEngine) {
    ...
    let prod_str = match (prod_name, prod_cost) {
        (Some(n), Some(cost)) => format!("{}/{} [{}]", city.production_stored, cost, capitalize(n)),
        _ => format!("{}/--", city.production_stored),
    };
    ...
    println!(
        "+-- {} --+",
        &sep[..width.saturating_sub(line1.len() + 6).min(width)]
    );
    println!("{line1}");
    println!("{line2}");
    println!("{line3}");
...
fn hline(title: &str) {
    println!(
        "  +-- {} {}",
        title,
        "-".repeat(40usize.saturating_sub(title.len()).max(4))
    );
}
    ...
    match item {
        ProductionItem::Unit(tid) => {
            match session.state.unit_type_defs.iter().find(|d| d.id == *tid) {
                Some(def) => (
                    def.name.to_string(),
                    format!("({} prod)", def.production_cost),
                ),
...
fn print_ai_turn_events(state: &GameState, ai_civ_id: CivId, diff: &GameStateDiff) {
    ...
    print!("  [{civ_name}]");
    if moves > 0 {
        print!("  {moves} unit move(s)");
    }
    if attacks > 0 {
        print!("  {attacks} attack(s)");
    }
    if founded > 0 {
        print!("  founded {founded} city/cities");
    }
    println!();
}
...
fn cmd_spread(session: &mut Session, rules: &DefaultRulesEngine) {
    ...
    match rules.spread_religion(&mut session.state, uid) {
        Ok(diff) => {
            for delta in &diff.deltas {
                if let StateDelta::ReligionSpread {
                    city,
                    followers_added,
                    ..
                } = delta
...
fn cmd_theo_combat(session: &mut Session, rules: &DefaultRulesEngine, q: i32, r: i32) {
    ...
    let defender = session
        ...
        .iter()
        .find(|u| {
            u.coord() == target_coord
                && u.owner != session.civ_id
                && u.category == UnitCategory::Religious
        })
        .map(|u| u.id);
    match defender {
        None => println!(
            "  [error] No enemy religious unit at {}.",
            fmtc(target_coord)
        ),
        Some(def_id) => match rules.theological_combat(&mut session.state, uid, def_id) {
            Ok(diff) => {
                for delta in &diff.deltas {
                    if let StateDelta::TheologicalCombat {
                        attacker_damage,
                        defender_damage,
                        ..
                    } = delta
                    {
                        println!(
                            "  [Theological] dealt {defender_damage} dmg, took {attacker_damage} dmg"
                        );
                    }
                    if let StateDelta::UnitDestroyed { .. } = delta {
                        println!("  Unit destroyed!");
...
fn cmd_retire(session: &mut Session, rules: &DefaultRulesEngine, name: &str) {
    ...
    let found = session
        ...
        .iter()
        .find(|gp| {
            gp.name.to_ascii_lowercase() == name_lower
                && gp.owner == Some(session.civ_id)
                && !gp.is_retired
        })
        .map(|gp| gp.id);
    match found {
...
fn cmd_great_work(session: &mut Session, rules: &DefaultRulesEngine, name: &str) {
    ...
    match found {
        ...
        Some(gp_id) => match rules.create_great_work(&mut session.state, gp_id) {
            Ok(diff) => {
                for delta in &diff.deltas {
                    if let StateDelta::GreatWorkCreated {
                        work_name, city, ..
                    } = delta
                    {
...
fn cmd_remove_heresy(session: &mut Session, rules: &DefaultRulesEngine) {
    ...
    match rules.remove_heresy(&mut session.state, uid) {
        Ok(diff) => {
            for delta in &diff.deltas {
                if let StateDelta::HeresyRemoved {
                    followers_removed, ..
                } = delta
                {
...
fn cmd_barb_hire(session: &mut Session, rules: &DefaultRulesEngine, q: i32, r: i32) {
    match find_camp_at(&session.state, q, r) {
        ...
        Some(camp_id) => {
            match rules.hire_from_barbarian_camp(&mut session.state, camp_id, session.civ_id) {
                Ok(diff) => {
                    for delta in &diff.deltas {
...
fn cmd_barb_bribe(session: &mut Session, rules: &DefaultRulesEngine, q: i32, r: i32) {
    match find_camp_at(&session.state, q, r) {
        None => println!("  [error] No barbarian camp at ({q},{r})."),
        Some(camp_id) => {
            match rules.bribe_barbarian_camp(&mut session.state, camp_id, session.civ_id) {
                Ok(diff) => {
                    for delta in &diff.deltas {
...
fn cmd_barb_incite(
    ...
    match find_camp_at(&session.state, q, r) {
        None => println!("  [error] No barbarian camp at ({q},{r})."),
        Some(camp_id) => match rules.incite_barbarian_camp(
            &mut session.state,
            camp_id,
            session.civ_id,
            target_id,
