...
use libciv::{BuiltinVictoryCondition, DefaultRulesEngine, GreatPersonType, RulesEngine};
...
fn add_slots_to_city(
    s: &mut common::Scenario,
    city_id: libciv::CityId,
    slots: Vec<GreatWorkSlotType>,
) {
...
fn test_domestic_culture_accumulates() {
    ...
    let civ = s
        ...
        .unwrap();
    assert!(
        civ.domestic_culture > 0,
        "domestic_culture should accumulate over turns"
    );
}
...
fn test_tourism_from_great_works() {
    ...
    add_slots_to_city(
        &mut s,
        rc,
        vec![GreatWorkSlotType::Writing, GreatWorkSlotType::Writing],
    );
    ...
    let civ = s
        ...
        .unwrap();
    assert!(
        civ.tourism_output >= 2,
        "writing great work should produce at least 2 tourism, got {}",
        civ.tourism_output
    );
}
...
fn test_cultural_victory_fires() {
    ...
    s.state
        .victory_conditions
        .push(BuiltinVictoryCondition::Culture { id: victory_id });
    ...
    assert!(
        ...
        "game should be over with cultural victory"
    );
...
fn test_cultural_victory_requires_exceeding_all_civs() {
    ...
    let writer_id = add_great_writer(&mut s, "Homer", rome_id);
    DefaultRulesEngine
        .create_great_work(&mut s.state, writer_id)
        .unwrap();
    ...
    assert!(
        ...
        "should not win when opponent has higher domestic culture"
    );
}
...
fn test_cultural_victory_not_fired_when_tied() {
    ...
    assert!(
        !progress.is_won(),
        "tourism == domestic_culture should not win (must strictly exceed)"
    );
}
