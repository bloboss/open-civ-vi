...
use libciv::civ::governor::{get_governor_modifiers, promotion_def, promotions_for};
...
fn test_assign_governor_to_city() {
    ...
    let diff = rules()
        .assign_governor(&mut s.state, gov_id, s.rome_city)
        .unwrap();
...
fn test_reassign_governor_resets_timer() {
    ...
    s.state
        ...
        .find(|g| g.id == gov_id)
        .unwrap()
        .turns_to_establish = 1;

    // Found a second city for Rome.
    let city2_id = s.state.id_gen.next_city_id();
    let city2 = libciv::civ::City::new(
        city2_id,
        "Ostia".into(),
        s.rome_id,
        libhexgrid::coord::HexCoord::from_qr(5, 5),
    );
    ...
    rules()
        .assign_governor(&mut s.state, gov_id, city2_id)
        .unwrap();
    ...
    assert_eq!(
        ...
        "reassignment should reset the timer"
    );
}
...
fn test_governor_loyalty_bonus_only_when_established() {
    ...
    let delta_unestablished: Vec<_> = diff1
        .deltas
        .iter()
    ...
    let delta_established: Vec<_> = diff2
        .deltas
        .iter()
        .filter_map(|d| match d {
            StateDelta::LoyaltyChanged { city, delta, .. } if *city == s.rome_city => Some(*delta),
...
fn test_promote_governor_basic() {
    ...
    s.state
        ...
        .governor_titles += 1;
    rules()
        .promote_governor(&mut s.state, gov_id, "Connoisseur")
        .unwrap();

    let diff = rules()
        .promote_governor(&mut s.state, gov_id, "Librarian")
        .unwrap();
...
fn test_governor_modifiers_only_when_established() {
    ...
    assert!(
        !mods.is_empty(),
        "established Pingala should have base modifiers"
    );
}
...
fn test_promotion_definitions_exist() {
    ...
    for name in libciv::civ::GOVERNOR_NAMES {
        ...
        assert_eq!(
            promos.len(),
            6,
            "governor {name} should have 6 promotions, got {}",
            promos.len()
        );
    }
}
...
fn test_civic_grants_governor_title() {
    ...
    let has_civic_completed = diff.deltas.iter().any(|d| {
        matches!(d,
    ...
    let has_title = diff.deltas.iter().any(|d| {
        matches!(d,
