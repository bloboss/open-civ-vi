...
fn discovering_natural_wonder_emits_delta() {
    ...
    let discovered = diff.deltas.iter().find(|d| {
        matches!(d,
...
fn rediscovering_natural_wonder_does_not_emit_again() {
    ...
    let count1 = diff1
        .deltas
        .iter()
        .filter(|d| matches!(d, StateDelta::NaturalWonderDiscovered { .. }))
        .count();
    assert_eq!(
        count1, 1,
        "First call should emit exactly one NaturalWonderDiscovered"
    );
    ...
    assert_eq!(
        count2, 0,
        "Second call should NOT emit NaturalWonderDiscovered again"
    );
}
...
fn wonder_discovery_grants_era_score_via_advance_turn() {
    ...
    let discovered = diff.deltas.iter().any(|d| {
        matches!(d,
    ...
    assert!(
        discovered,
        "advance_turn should emit NaturalWonderDiscovered"
    );
    ...
    assert!(
        has_moment,
        "advance_turn should emit HistoricMomentEarned for wonder discovery"
    );
    ...
    assert!(
        ...
        "era score should increase after wonder discovery"
    );
}
