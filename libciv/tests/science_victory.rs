...
fn complete_all_milestones_wins_science_victory() {
    ...
    s.state
        .victory_conditions
        .push(BuiltinVictoryCondition::Science { id: vc_id });

    // Complete all 3 milestones for Rome.
    for i in 0..SCIENCE_MILESTONES.len() {
        let diff = rules
            .complete_science_milestone(&mut s.state, s.rome_id)
        ...
        assert!(
            found,
            "expected ScienceMilestoneCompleted delta for {:?}",
            SCIENCE_MILESTONES[i]
        );
    }
...
fn partial_milestones_do_not_trigger_victory() {
    ...
    assert!(
        ...
        "game should not be over with only 2 milestones"
    );
}

#[test]
fn fifth_milestone_returns_error() {
    let rules = DefaultRulesEngine;
    let mut s = common::build_scenario();

    // Complete all 4 milestones (GS adds Exoplanet Expedition as 4th).
    for _ in 0..4 {
        rules
            .complete_science_milestone(&mut s.state, s.rome_id)
            .unwrap();
    ...
    // A fifth attempt should fail.
    let err = rules.complete_science_milestone(&mut s.state, s.rome_id);
    assert!(
        matches!(err, Err(RulesError::AllMilestonesCompleted)),
        "expected AllMilestonesCompleted error, got {:?}",
        err
    );
}
