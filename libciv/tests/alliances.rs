...
fn form_alliance_sets_status() {
    ...
    let diff = rules
        .form_alliance(&mut state, s.rome_id, s.babylon_id, AllianceType::Research)
    ...
    assert!(diff.deltas.iter().any(|d| matches!(
        d,
        libciv::game::diff::StateDelta::AllianceFormed {
            alliance_type: AllianceType::Research,
            ..
        }
    )));
    assert!(diff.deltas.iter().any(|d| matches!(
        d,
        libciv::game::diff::StateDelta::DiplomacyChanged {
...
fn alliance_levels_up_over_time() {
    ...
    rules
        .form_alliance(&mut state, s.rome_id, s.babylon_id, AllianceType::Military)
        .expect("form_alliance should succeed");
    ...
    assert_eq!(
        ...
        "alliance should be level 2 after 30 turns"
    );
    ...
    assert_eq!(
        ...
        "alliance should be level 3 after 60 turns"
    );
...
fn cannot_ally_while_at_war() {
    ...
    // Declare war first
    rules
        .declare_war(&mut state, s.rome_id, s.babylon_id)
