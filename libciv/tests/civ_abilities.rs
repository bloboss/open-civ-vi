...
use libciv::{
    CivId, DefaultRulesEngine, GreatPersonType, RulesEngine, UnitCategory, UnitDomain, UnitTypeId,
};
...
fn test_rome_city_founded_gets_monument_and_trading_post() {
    ...
    let diff = rules
        .found_city(&mut s.state, settler_id, "New Roma".to_string())
        .unwrap();
    ...
    assert!(
        has_building_delta,
        "should emit BuildingCompleted for the free Monument"
    );
...
fn test_babylon_eureka_gives_full_tech() {
    ...
    s.state
        ...
        .unwrap()
        .research_queue
        .push_back(libciv::civ::TechProgress {
            tech_id: pottery_id,
            progress: 0,
            boosted: false,
        });
...
fn test_greece_extra_wildcard_slot() {
    ...
    let has_wildcard = bundle.rule_overrides.iter().any(|o| {
        matches!(
            o,
            libciv::civ::RuleOverride::ExtraPolicySlot(libciv::PolicyType::Wildcard)
        )
    });
    assert!(
        has_wildcard,
        "Greece should have ExtraPolicySlot(Wildcard) override"
    );
...
fn test_germany_extra_district_slot() {
    ...
    let has_extra = bundle
        .rule_overrides
        .iter()
        .any(|o| matches!(o, libciv::civ::RuleOverride::ExtraDistrictSlot(1)));
    assert!(
        has_extra,
        "Germany should have ExtraDistrictSlot(1) override"
    );
}
...
fn test_hoplite_adjacency_bonus() {
    ...
    let atk_delta = diff
        .deltas
        .iter()
        .find(|d| matches!(d, StateDelta::UnitAttacked { .. }));
    assert!(
        atk_delta.is_some(),
        "attack should produce UnitAttacked delta"
    );

    // The warrior (20 CS) should take significant damage from effective 38 CS hoplite.
    if let Some(StateDelta::UnitAttacked {
        defender_damage, ..
    }) = atk_delta
    {
        assert!(
...
fn test_samurai_no_damage_penalty() {
    ...
    assert!(
        samurai
            .abilities
            .contains(&libciv::rules::unique::UniqueUnitAbility::NoCombatPenaltyWhenDamaged),
...
fn test_varu_debuffs_adjacent_enemies() {
    ...
    assert!(
        varu.abilities
            .contains(&libciv::rules::unique::UniqueUnitAbility::DebuffAdjacentEnemies(5)),
...
fn test_mamluk_heals_every_turn() {
    ...
    assert_eq!(
        mamluk.health, 80,
        "Mamluk should heal 10 (ability) + 10 (idle) HP per turn"
    );
}
...
fn test_stepwell_is_india_exclusive() {
    ...
    assert_eq!(
        stepwell.adjacency_bonuses.len(),
        2,
        "Stepwell should have 2 adjacency bonuses"
    );
}
