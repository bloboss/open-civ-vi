...
fn project_completes_after_production() {
    ...
    let city = s
        ...
        .unwrap();
    city.districts
        .push(libciv::civ::district::BuiltinDistrict::Campus);
    ...
    let completed = diff.deltas.iter().any(|d| {
        matches!(
    ...
    assert!(
        completed,
        "expected ProjectCompleted delta for Campus Research Grants"
    );
    ...
    assert!(
        ...
        "queue should be empty after project completes"
    );
}
...
fn science_project_increments_milestones() {
    ...
    let city = s
        ...
        .unwrap();
    city.districts
        .push(libciv::civ::district::BuiltinDistrict::Spaceport);
    ...
    assert_eq!(
        new_milestones,
        initial + 1,
        "science milestones should increment by 1"
    );
    ...
    assert!(
        has_milestone_delta,
        "expected ScienceMilestoneCompleted delta"
    );
}
...
fn carbon_recapture_reduces_co2() {
    ...
    let city = s
        ...
        .unwrap();
    city.districts
        .push(libciv::civ::district::BuiltinDistrict::IndustrialZone);
    ...
    assert_eq!(
        s.state.global_co2, 150,
        "Carbon Recapture should reduce CO2 by 50"
    );
}
...
fn non_repeatable_project_removed_from_defs_after_completion() {
    ...
    let still_exists = s
        ...
        .project_defs
        .iter()
        .any(|d| d.name == "Launch Earth Satellite");
    assert!(
        !still_exists,
        "non-repeatable project should be removed from defs"
    );
...
fn repeatable_project_stays_in_defs() {
    ...
    let still_exists = s
        ...
        .project_defs
        .iter()
        .any(|d| d.name == "Campus Research Grants");
