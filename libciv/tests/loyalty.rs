...
fn loyalty_deltas(diff: &libciv::GameStateDiff) -> Vec<(libciv::CityId, i32, i32)> {
    diff.deltas
        .iter()
        .filter_map(|d| {
            if let StateDelta::LoyaltyChanged {
                city,
                delta,
                new_value,
            } = d
...
fn revolt_deltas(diff: &libciv::GameStateDiff) -> Vec<(libciv::CityId, Option<CivId>, CivId)> {
    diff.deltas
        .iter()
        .filter_map(|d| {
            if let StateDelta::CityRevolted {
                city,
                new_owner,
                old_owner,
            } = d
...
fn cities_start_at_full_loyalty() {
    ...
    for city in &s.state.cities {
        assert_eq!(
            city.loyalty, 100,
            "city {} should start at loyalty 100",
            city.name
        );
    }
}
...
fn isolated_capital_stays_loyal() {
    ...
    assert_eq!(
        rome_city.loyalty, 100,
        "isolated capital should stay at max loyalty"
    );
}
...
fn loyalty_changed_delta_emitted() {
    ...
    assert!(
        !changes.is_empty(),
        "expected at least one LoyaltyChanged delta"
    );
}
...
fn occupied_city_loses_loyalty_faster() {
    ...
    let nippur_change = loyalty_deltas(&diff)
        .into_iter()
    ...
    if let Some((_, delta, _)) = nippur_change {
        ...
        assert!(
            delta < 0,
            "occupied city should lose loyalty, got delta {delta}"
        );
    }
}
...
fn governor_stabilizes_loyalty() {
    ...
    let delta_no_gov = loyalty_deltas(&diff_no_gov)
        .into_iter()
    ...
    let delta_with_gov = loyalty_deltas(&diff_with_gov)
        .into_iter()
        .find(|(id, _, _)| *id == city_id)
        .map(|(_, d, _)| d)
        .unwrap_or(0);
...
fn city_ownership_transfers_on_revolt() {
    ...
    assert_eq!(
        ...
        "flipped city should be Occupied"
    );
    ...
    let rome = s
        ...
        .unwrap();
    assert!(
        rome.cities.contains(&nippur_id),
        "Rome should have Nippur in its city list"
    );
    ...
    let babylon = s
        ...
        .unwrap();
    assert!(
        !babylon.cities.contains(&nippur_id),
        "Babylon should no longer own Nippur"
    );
}
...
fn territory_transfers_on_revolt() {
    ...
    assert_eq!(
        tile.owner,
        Some(s.rome_id),
        "territory tile should flip to Rome"
    );
}
...
fn capital_resists_loyalty_loss() {
    ...
    assert!(
        rome_city.loyalty > 0,
        "capital should resist loyalty loss: loyalty={}",
        rome_city.loyalty
    );
}
...
fn city_states_skip_loyalty() {
    ...
    cs_city.kind = libciv::civ::CityKind::CityState(libciv::civ::CityStateData::new(
        libciv::civ::CityStateType::Cultural,
    ));
    cs_city.loyalty = 100;
...
fn loyalty_delta_is_clamped() {
    ...
    let antium_change = loyalty_deltas(&diff)
        .into_iter()
        .find(|(id, _, _)| *id == city_id);
    if let Some((_, delta, _)) = antium_change {
        assert!(
            delta >= -20,
            "loyalty delta should be clamped to -20, got {delta}"
        );
    }
}
...
fn sustained_pressure_causes_revolt() {
    ...
    for _ in 0..50 {
        let diff = rules.advance_turn(&mut s.state);
        if revolt_deltas(&diff)
            .iter()
            .any(|(id, _, _)| *id == nippur_id)
        {
            revolted = true;
            break;
        }
    ...
    assert!(
        revolted,
        "Nippur should eventually revolt under sustained foreign pressure"
    );
}
