...
fn diplomatic_favor_accumulates_each_turn() {
    ...
    assert!(
        rome_favor >= 2,
        "expected at least 2 favor after 1 turn, got {rome_favor}"
    );

    // Verify a DiplomaticFavorChanged delta was emitted.
    let has_favor_delta = diff
        .deltas
        .iter()
        .any(|d| matches!(d, StateDelta::DiplomaticFavorChanged { civ, .. } if *civ == s.rome_id));
    assert!(
        has_favor_delta,
        "expected DiplomaticFavorChanged delta for Rome"
    );
}
...
fn diplomatic_victory_fires_at_threshold() {
    ...
    s.state
        .victory_conditions
        .push(BuiltinVictoryCondition::Diplomatic {
            id: vc_id,
            threshold: 10,
        });
    ...
    assert!(
        ...
        "game should be over once VP >= threshold"
    );
...
fn war_reduces_favor_gain() {
    ...
    assert_eq!(
        rome_favor, 1,
        "expected 1 favor while at war, got {rome_favor}"
    );
}
