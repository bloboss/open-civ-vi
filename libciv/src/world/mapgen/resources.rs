...
pub fn generate(board: &mut WorldBoard, zone_map: &HashMap<HexCoord, EcoZone>, rng: &mut SmallRng) {
...
fn place_resource(
    ...
    for coord in board.all_coords() {
        ...
        if !def.zones.contains(&zone) {
            continue;
        }

        // Water vs land check.
        let tile_is_water = !is_land(tile.terrain);
        if def.water_only && !tile_is_water {
            continue;
        }
        if !def.water_only && tile_is_water {
            continue;
        ...
        // Terrain check.
        if !def.terrains.is_empty() && !def.terrains.contains(&tile.terrain) {
            continue;
        }
    ...
    for &coord in &candidates {
        ...
        let too_close = placed
            .iter()
            .any(|p: &HexCoord| p.distance(&coord) < min_sep);
        if too_close {
            continue;
        }
