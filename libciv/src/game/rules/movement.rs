...
pub(crate) fn move_unit(
    ...
    let full_path = match domain {
        UnitDomain::Land if can_coast => {
            state
                .board
                .find_path_embark(from, to_norm, u32::MAX, can_ocean)
        }
    ...
    for i in 1..full_path.len() {
        ...
        let Some(tile) = state.board.tile(next) else {
            break;
        };
        ...
        if domain == UnitDomain::Land {
            if next_is_water && !is_embarked {
                ...
                reached = next;
                spent = budget;
                diff.push(StateDelta::UnitDisembarked {
                    unit: unit_id,
                    coord: next,
                });
                break;
    ...
    if let Some(occupant) = state
        .units
        .iter()
        .find(|u| u.id != unit_id && u.coord == reached)
    {
        let mover_owner = state.unit(unit_id).map(|u| u.owner);
        let mover_can_attack = state
            .unit(unit_id)
            .and_then(|u| u.combat_strength)
            .is_some();
...
pub(crate) fn decrement_builder_charges(
    state: &mut GameState,
    unit_id: UnitId,
