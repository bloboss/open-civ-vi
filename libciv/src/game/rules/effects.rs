...
pub(crate) fn apply_effect(
    ...
    match effect {
        OneShotEffect::RevealResource(r) => {
            ...
            diff.push(StateDelta::ResourceRevealed {
                civ: civ_id,
                resource: *r,
            });
        }
        ...
        OneShotEffect::UnlockUnit(u) => {
            ...
            diff.push(StateDelta::UnitUnlocked {
                civ: civ_id,
                unit_type: u,
            });
        }
        ...
        OneShotEffect::UnlockImprovement(i) => {
            ...
            diff.push(StateDelta::ImprovementUnlocked {
                civ: civ_id,
                improvement: i,
            });
        }
        ...
        OneShotEffect::TriggerEureka { tech } => {
            ...
            let babylon_full_tech = lookup_bundle(civ_identity).is_some_and(|b| {
                b.rule_overrides
                    .iter()
            ...
            let tech_name = state.tech_tree.get(*tech).map(|n| n.name).unwrap_or("?");
            diff.push(StateDelta::EurekaTriggered {
                civ: civ_id,
                tech: tech_name,
            });
        }
        ...
        OneShotEffect::TriggerInspiration { civic } => {
            ...
            diff.push(StateDelta::InspirationTriggered {
                civ: civ_id,
                civic: civic_name,
            });
        }

        OneShotEffect::FreeUnit {
            unit_type,
            city: hint_city,
        } => {
            let coord = hint_city
                ...
                .cloned()
            {
                ...
                diff.push(StateDelta::FreeUnitGranted {
                    civ: civ_id,
                    unit_type,
                    coord,
                });
        ...
        OneShotEffect::FreeBuilding { building, city } => {
            ...
            if let Some(target_cid) = target_city_id {
                    ...
                    .cloned()
                {
                    ...
                    let _ = def;
                    diff.push(StateDelta::FreeBuildingGranted {
                        civ: civ_id,
                        building,
                        city: target_cid,
                    });
        ...
        OneShotEffect::UnlockGovernment(g) => {
            ...
            diff.push(StateDelta::GovernmentUnlocked {
                civ: civ_id,
                government: g,
            });
        }
        ...
        OneShotEffect::AdoptGovernment(g) => {
            ...
            if let Some(new_gov) = new_gov {
                ...
                let active: Vec<crate::PolicyId> =
                    state.civilizations[civ_idx].active_policies.clone();
                let mut kept = Vec::new();
                let mut removed = Vec::new();

                for pid in active {
                    let policy_type = state
                        .policies
                        .iter()
                        .find(|p| p.id == pid)
                        .map(|p| p.policy_type);
                    let fits = match policy_type {
                        Some(PolicyType::Military) if mil_free > 0 => {
                            mil_free -= 1;
                            true
                        }
                        Some(PolicyType::Economic) if eco_free > 0 => {
                            eco_free -= 1;
                            true
                        }
                        Some(PolicyType::Diplomatic) if dip_free > 0 => {
                            dip_free -= 1;
                            true
                        }
                        Some(PolicyType::Wildcard) if wc_free > 0 => {
                            wc_free -= 1;
                            true
                        }
                        _ => false,
                ...
                for pid in removed {
                    diff.push(StateDelta::PolicyUnslotted {
                        civ: civ_id,
                        policy: pid,
                    });
                }
            ...
            diff.push(StateDelta::GovernmentAdopted {
                civ: civ_id,
                government: g,
            });
        }
        ...
        OneShotEffect::UnlockPolicy(p) => {
            ...
            diff.push(StateDelta::PolicyUnlocked {
                civ: civ_id,
                policy: p,
            });
        }
