...
pub fn run_demo_game(
    ...
    for (i, &(name, adjective, leader_name, _color)) in
        CIV_POOL.iter().enumerate().take(num_players)
    {
        ...
        let initial: Vec<HexCoord> = std::iter::once(coord)
            ...
            .collect();
        for &c in &initial {
            if let Some(tile) = state.board.tile_mut(c) {
    ...
    let spectator_view =
        |state: &GameState, civ_ids: &[CivId]| -> GameView { project_game_view(state, civ_ids[0]) };
    ...
    {
        let mut views: Vec<GameView> = civ_ids
            .iter()
            .map(|&cid| project_game_view(&state, cid))
            .collect();
